...
export const AIMentorModal: React.FC<AIMentorModalProps> = ({
  ...
  const [activeMode, setActiveMode] = useState<"evaluate" | "strategy" | "explain">(initialMode);
  ...
  const handleGenerateStrategy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStrategyResult("Strategy generation encountered an error. Please try again.");
    } finally {
  ...
  const handleExplainTopic = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setExplainResult("Topic explainer encountered an error. Please try again.");
    } finally {
        ...
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">Topper Answer Rubrics & Evaluation</p>
            </div>
          </div>
              ...
              <form onSubmit={handleEvaluateAnswer} className="space-y-4">
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">UPSC Mains Question</label>
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-slate-500 font-semibold mr-1">Marks:</span>
                      {[10, 15].map(m => (
                        <button
                          key={m}
                          type="button"
                          ...
                          className={`px-2.5 py-1 rounded-md text-[11px] font-bold cursor-pointer transition ${
                            targetMarks === m ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                          }`}
                        >
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Your Written Answer</label>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pt-2">
                  <span className="text-[11px] text-slate-500 italic">Evaluates structure, intro, SC rulings, keywords, and tone.</span>
                  <button
                    type="submit"
              ...
              {evalResult && (
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 space-y-5 animate-fade-in shadow-xs">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-slate-500">Score & Assessment</span>
                      <div className="text-2xl sm:text-3xl font-extrabold text-indigo-600 mt-0.5">
                        {evalResult.estimatedScore} <span className="text-base text-slate-500 font-normal">/ {evalResult.maxMarks} Marks</span>
                      </div>
                    </div>

                    <div className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-left sm:text-right shadow-xs">
                      <span className="text-[10px] uppercase font-bold text-slate-500">Readiness Category</span>
                      <div className="text-sm font-extrabold text-emerald-700">Topper League Standard</div>
                    </div>
                  </div>

                  {/* Rubric Breakdown Grid */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                    {Object.entries(evalResult.rubricBreakdown).map(([key, val]) => (
                      <div key={key} className="p-3 bg-white rounded-xl border border-slate-200 text-center shadow-xs">
                        <div className="text-[10px] font-bold text-slate-500 capitalize">{key.replace(/([A-Z])/g, " $1")}</div>
                        <div className="text-base font-extrabold text-indigo-600 mt-0.5">{val}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-2 shadow-xs">
                    <div className="text-xs font-bold text-amber-800 uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      <span>Topper-Level Upgrades (SC Judgments, Data, Case Laws)</span>
                    </div>
                    <ul className="text-xs text-slate-700 space-y-1.5 font-medium">
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-700 leading-relaxed space-y-1 shadow-xs">
                    <strong className="text-indigo-600 block font-bold">Recommended Model Structure: </strong>
                    <p className="font-medium">{evalResult.modelAnswerOutline}</p>
                  </div>
            ...
            <div className="space-y-5">
              <form onSubmit={handleGenerateStrategy} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">Academic Background</label>
                    <select
                    ...
                    >
                      ...
                      <option value="Commerce / CA / Finance">Commerce / CA / Finance</option>
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">Target Examination Year</label>
                    <select
                      value={targetYear}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                  <div className="flex items-center gap-2 text-xs text-slate-700 font-medium">
                    <input
                      ...
                      onChange={(e) => setIsWorkingProfessional(e.target.checked)}
                      className="rounded accent-indigo-600 cursor-pointer"
                    />
                    <label htmlFor="working-prof" className="cursor-pointer">I am a Working Professional (4-5 hrs/day constraint)</label>
                  </div>
                  ...
                  >
                    {isGeneratingStrategy ? <RotateCcw className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
            ...
            <div className="space-y-5">
              <form onSubmit={handleExplainTopic} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">Concept / Topic Name</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">GS Paper Context</label>
                    <select
                      value={topicPaper}
                      onChange={(e) => setTopicPaper(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-300 text-slate-900 text-xs sm:text-sm rounded-xl p-3 outline-none focus:border-indigo-600"
                    >
                      <option value="GS1 Heritage, History, Geography, Society">GS1 (Hist, Geo, Society)</option>
                      <option value="GS2 Polity, Governance, Constitution, IR">GS2 (Polity, Gov, IR)</option>
                      <option value="GS3 Economy, Environment, Sci-Tech, Security">GS3 (Economy, Env, Sec)</option>
                      <option value="GS4 Ethics, Integrity and Aptitude">GS4 (Ethics, Case Studies)</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 text-white font-bold text-xs hover:bg-indigo-700 disabled:opacity-50 transition cursor-pointer shadow-sm"
                  >
                    {isExplaining ? <RotateCcw className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
