...
const newRoutines = data.TOPPERS_PROFILES.map((topper, index) => {
  const template = templates[index % templates.length];
  
  return {
    id: `routine_${topper.id}`,
    title: `Routine of ${topper.name} (AIR ${topper.rank})`,
    topperRef: topper.name,
    type: template.type,
    ...
    extraData: {
      "Optional Subject": topper.optional,
...
console.log(`Mapped ${newRoutines.length} routines to actual toppers successfully!`);
