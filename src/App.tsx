...
import { DEFAULT_REVISION_QUEUE, DEFAULT_MOCK_LOGS, DEFAULT_WEAK_AREAS } from "./data/analyticsDefaults";
...
export default function App() {
  // Navigation State
  const [activeTab, setActiveTab] = useState<MainTab>("toppers");
  ...
  const [sessionLogs, setSessionLogs] = useState<StudySessionLog[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      ...
      },
      {
        ...
        paper: "Prelims GS1",
        durationMinutes: 60,
        topicCovered: "The Hindu Editorial: India-US Trade & Semiconductor Mission",
        qualityRating: 4
      }
    ];
  });
  ...
  const [weakAreas, setWeakAreas] = useState<WeakAreaItem[]>(DEFAULT_WEAK_AREAS);
  ...
  const [aiModalInitialMode, setAiModalInitialMode] = useState<"evaluate" | "strategy" | "explain">("evaluate");
        ...
        setTimerSeconds(prevSeconds => {
          ...
          if (prevSeconds + 1 >= targetSecs) {
            ...
            if (timerConfig.nativeNotificationsEnabled !== false) {
              ...
              playTimerChime(timerPhase === "focus" ? "focus_end" : "break_end");
            }
  ...
  const handleUpdateTopicStatus = (topicId: string, nextStatus: SyllabusTopic["status"]) => {
  ...
  const handleToggleMilestone = (phaseId: string, milestoneId: string) => {
    setStudyPlanPhases(prev => prev.map(p => {
  ...
  const handleAddMilestone = (phaseId: string, title: string, targetDate: string) => {
    setStudyPlanPhases(prev => prev.map(p => {
      if (p.id === phaseId) {
        return {
          ...p,
  ...
  const handleResetDefaultRevisionQueue = () => {
    ...
    localStorage.setItem("upsc_revision_queue", JSON.stringify(DEFAULT_REVISION_QUEUE));
  };
  ...
  const handleGlobalNavigate = (tab: MainTab, subTab?: string) => {
    ...
    if (tab === "analytics" && subTab) setAnalyticsSubTab(subTab as AnalyticsSubTab);
  };
            ...
            >
              ...
              {activeTab === "prep" && (
                <PrepSection
                  ...
                  onResetDefaultRevisionQueue={handleResetDefaultRevisionQueue}
                  pyqs={pyqs}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-lg border-t border-slate-200 shadow-xl safe-bottom px-2 py-1.5 flex items-center justify-around">
        <button
          ...
          className={`flex flex-col items-center justify-center py-1 px-3 rounded-xl transition cursor-pointer ${
            activeTab === "toppers" ? "text-indigo-600 font-bold bg-indigo-50/80" : "text-slate-500 hover:text-slate-800"
          }`}
        >
          <Trophy className={`w-5 h-5 mb-0.5 ${activeTab === "toppers" ? "text-indigo-600" : "text-slate-500"}`} />
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center py-1 px-3 rounded-xl transition cursor-pointer ${
            activeTab === "prep" ? "text-indigo-600 font-bold bg-indigo-50/80" : "text-slate-500 hover:text-slate-800"
          }`}
        >
          <BookOpen className={`w-5 h-5 mb-0.5 ${activeTab === "prep" ? "text-indigo-600" : "text-slate-500"}`} />
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center py-1 px-3 rounded-xl transition cursor-pointer ${
            activeTab === "analytics" ? "text-indigo-600 font-bold bg-indigo-50/80" : "text-slate-500 hover:text-slate-800"
          }`}
        >
          <BarChart3 className={`w-5 h-5 mb-0.5 ${activeTab === "analytics" ? "text-indigo-600" : "text-slate-500"}`} />
      ...
      <footer className="border-t border-slate-200 bg-white py-6 text-slate-500 text-xs text-center mt-12 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className="w-5 h-5 rounded-md bg-indigo-600 text-white font-bold flex items-center justify-center text-[10px]">U</span>
            <span className="font-semibold text-slate-700">UPSC CONQUEST Master Hub</span>
            <span>• Bento Grid Preparation Suite</span>
          </div>
          <div className="flex items-center gap-4 text-[11px] text-slate-500">
            <span>Union Public Service Commission (CSE)</span>
            <span className="text-indigo-600 font-bold">• 100% Offline-Safe Storage</span>
          </div>
        </div>
      </footer>
