...
export const QuickStudyLogModal: React.FC<QuickStudyLogModalProps> = ({
  ...
  const suggestedTopics = syllabus
    .filter(s => s.subject.toLowerCase().includes(subject.toLowerCase().slice(0, 5)) || subject.toLowerCase().includes(s.subject.toLowerCase().slice(0, 5)))
    .slice(0, 4);
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 font-mono">Updates Daily Progress</span>
              </div>
              <h3 className="text-lg font-bold text-slate-900 mt-0.5">Quick Study Session Log</h3>
            </div>
          </div>
        ...
        {showSuccess ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-3 text-center animate-in zoom-in-95 duration-200">
            ...
            <h4 className="text-base font-bold text-slate-900">Study Session Logged!</h4>
            <p className="text-xs text-slate-500 font-medium">
              +{durationMinutes} mins added to today's study telemetry and streak tracking.
            </p>
          </div>
        ) : (
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">
                  Topic / Chapter Studied <span className="text-rose-500">*</span>
                </label>
                <span className="text-[10px] text-slate-400 font-mono">e.g. Fundamental Rights Art 19-22</span>
              </div>
              <input
                type="text"
                required
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-xs font-mono font-extrabold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-lg border border-indigo-100">
                  {durationMinutes} Minutes ({(durationMinutes / 60).toFixed(1)} hrs)
                </span>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 bg-slate-50 p-2 rounded-xl border border-slate-200">
                  {([1, 2, 3, 4, 5] as const).map((star) => (
                    ...
                    >
                      <Star className={`w-4 h-4 ${qualityRating >= star ? "fill-amber-400" : ""}`} />
                    </button>
                  ))}
